package com.example.transportfirm.controller.driver;
...
import com.example.transportfirm.service.driver.DriverDocumentService;
import com.example.transportfirm.service.driver.DriverService;
...
public class DriverController {
    ...
    public ResponseEntity<DriverInfo> updateDriver(
        ...
        return ResponseEntity.ok(driverService.update(driverInfoId, driver));
    }

    @DeleteMapping("/{driverInfoId}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void deleteDriver(@PathVariable UUID driverInfoId) {
        driverService.delete(driverInfoId);
    }
    ...
    public ResponseEntity<List<DriverDocument>> getDocuments(@PathVariable UUID employeeId) {
        return ResponseEntity.ok(driverDocumentService.getByEmployee(employeeId));
    }

    @GetMapping("/documents/{docId}/download")
    public ResponseEntity<byte[]> downloadDocument(@PathVariable UUID docId) throws IOException {
        DriverDocument doc = driverDocumentService.getById(docId);
        byte[] fileBytes = driverDocumentService.getBytes(docId);
    ...
    public DriverDocument upload(
        ...
        return driverDocumentService.add(employeeId, type, file);
    }

    @DeleteMapping("/documents/{docId}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public void deleteDoc(@PathVariable UUID docId) {
        driverDocumentService.delete(docId);
    }
}
